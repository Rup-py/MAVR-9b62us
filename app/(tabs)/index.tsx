...
import { AvatarFrame, RankBadge, TrainerBadge, CheckInBar, StatBar } from '@/components';
    ...
    <View style={[gaugeStyles.container, isDrifting && gaugeStyles.driftMode]}>
      ...
      <View style={gaugeStyles.track}>
        {Array.from({ length: 10 }).map((_, i) => {
          ...
          return (
            <View key={i} style={[gaugeStyles.segment, filled && { backgroundColor: segColor }]} />
          );
        })}
      </View>
...
const gaugeStyles = StyleSheet.create({
  container: {
    ...
    padding: Spacing.lg, borderWidth: 1, borderColor: Colors.Primary + '44', gap: Spacing.md,
  },
  driftMode: { borderColor: '#F59E0B44', backgroundColor: '#111108' },
  ...
  driftAlert: {
    ...
    borderWidth: 1, borderColor: '#F59E0B44', marginBottom: 8,
  },
  ...
  segment: { flex: 1, height: 8, borderRadius: 2, backgroundColor: Colors.SurfaceElevated },
...
function SmartWatchPanel() {
  ...
  const handleConnect = () => {
    setScanning(true);
    Animated.loop(Animated.timing(scanAnim, { toValue: 1, duration: 1200, useNativeDriver: true })).start();
    setTimeout(() => {
      setScanning(false);
...
const watchStyles = StyleSheet.create({
  ...
  connectBtn: {
    ...
    borderWidth: 1, borderColor: Colors.Primary + '55', borderRadius: Radius.md,
    paddingVertical: 14, backgroundColor: Colors.PrimaryGlow,
  },
  ...
  connected: {
    backgroundColor: Colors.SurfaceCard, borderRadius: Radius.xl,
    padding: Spacing.lg, borderWidth: 1, borderColor: '#22C55E33', gap: Spacing.md,
  },
  ...
  metric: { width: '30%', flex: 1, backgroundColor: Colors.SurfaceElevated, borderRadius: Radius.md, padding: Spacing.sm, gap: 2, minWidth: 90 },
  ...
  readinessCard: {
    ...
    borderRadius: Radius.lg, padding: Spacing.md, borderWidth: 1, borderColor: Colors.SurfaceBorder,
  },
...
const volStyles = StyleSheet.create({
  ...
  prBadge: { backgroundColor: Colors.Primary, borderRadius: Radius.full, paddingHorizontal: 8, paddingVertical: 3 },
  prText: { fontSize: 9, color: '#fff', fontWeight: FontWeight.black, letterSpacing: 1.5 },
  grid: { flexDirection: 'row', gap: Spacing.sm },
  metric: {
    flex: 1, backgroundColor: Colors.SurfaceElevated,
    borderRadius: Radius.md, padding: Spacing.sm, gap: 1, alignItems: 'center',
  },
  ...
  restTimer: {
    ...
    padding: Spacing.sm, borderWidth: 1, borderColor: Colors.Primary + '33',
  },
  ...
  return (
    <View style={nutStyles.container}>
      ...
      <View style={nutStyles.budgetRow}>
        ...
        <Text style={nutStyles.budgetSpent}>Rs.{spent} spent</Text>
        <Text style={[nutStyles.budgetRemain, { color: '#22C55E' }]}>Rs.{remaining} left</Text>
      </View>
      <View style={nutStyles.budgetTrack}>
        <View style={[nutStyles.budgetFill, { width: `${(spent / budget) * 100}%` }]} />
      </View>
      <Text style={nutStyles.aiSuggest}>
        AI suggests: Chicken Breast 150g + Rice 100g — best macro-per-rupee with Rs.{remaining} remaining.
      </Text>
    </View>
  );
}
...
function RankVelocityPanel({ user }: { user: any }) {
  ...
  const daysNeeded = Math.max(0, 14 - Math.floor(velocity / 10));
  ...
  return (
    <View style={rankStyles.container}>
      ...
      <View style={rankStyles.grid}>
        ...
        <View style={rankStyles.velMetric}>
          <Text style={[rankStyles.velVal, { color: '#FFD700' }]}>{streak}d</Text>
      ...
      <Text style={rankStyles.unlockText}>
        {daysNeeded} more days at more than 90% adherence unlock ASCEND tier + Elite apparel access.
      </Text>
    </View>
  );
}
...
const rankStyles = StyleSheet.create({
  container: {
    backgroundColor: Colors.SurfaceCard, borderRadius: Radius.xl,
    padding: Spacing.md, borderWidth: 1, borderColor: '#FFD70033', gap: Spacing.sm,
  },
  header: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  ...
  velMetric: {
    ...
    backgroundColor: Colors.SurfaceElevated, borderRadius: Radius.md, padding: Spacing.sm, gap: 2,
  },
...
// ── Unlock Banner ─────────────────────────────────────────────────────────────
function UnlockBanner({ onPress }: { onPress: () => void }) {
  const pulseAnim = useRef(new Animated.Value(1)).current;
  useEffect(() => {
    Animated.loop(Animated.sequence([
  ...
  return (
    <Animated.View style={{ transform: [{ scale: pulseAnim }] }}>
      <Pressable style={unlockBannerStyles.banner} onPress={onPress}>
        <MaterialIcons name="lock" size={20} color="#FFD700" />
        <View style={{ flex: 1 }}>
          <Text style={unlockBannerStyles.title}>UNLOCK FULL ACCESS</Text>
          <Text style={unlockBannerStyles.sub}>Enter product code to activate Connects, Vault, and Trainer Ecosystem</Text>
        </View>
...
const unlockBannerStyles = StyleSheet.create({
  banner: {
    ...
    backgroundColor: '#FFD70011', borderRadius: Radius.lg,
    padding: Spacing.md, borderWidth: 1, borderColor: '#FFD70044',
  },
  title: { fontSize: FontSize.sm, color: '#FFD700', fontWeight: FontWeight.black, letterSpacing: 1.5 },
...
export default function HomeScreen() {
  ...
  const { user, workoutPlan, dietPlan, isUnlocked } = useUser();
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: 120 }]} showsVerticalScrollIndicator={false}>
        ...
        {/* Unlock Banner */}
        {!isUnlocked && (
          <UnlockBanner onPress={() => router.push('/unlock')} />
        )}
        ...
        <View style={styles.nextActionCard}>
          ...
          <Text style={styles.nextActionSub}>{wp.today.day} · {wp.today.focus} · {wp.today.estimatedDuration} min</Text>
...
const styles = StyleSheet.create({
  ...
  nextActionCard: { backgroundColor: Colors.Primary, borderRadius: Radius.xl, padding: Spacing.lg, gap: Spacing.sm },
  ...
  nextActionBtn: {
    ...
    paddingVertical: 12, gap: 8, marginTop: Spacing.sm,
  },
  ...
  todayCard: {
    ...
    padding: Spacing.md, borderWidth: 1, borderColor: Colors.SurfaceBorder, gap: 6,
  },
